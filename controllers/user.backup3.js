...
exports.updateItem = (req, res, next) => {
    ...
    User.findOne(
      { _id:req.user.id },
      { list:1} ).lean().exec(function(err,user) {
       ...
       for (var ii = 0; ii < user.list.length; ii++) {
            for (var jj = 0; ii < user.list[ii].emails; jj++) {
                if(user.list[ii].items[jj].timeStampId == timeId ) { // update it below
                    user.list[ii].items[jj].picked == isPicked;
                    whichArrayToUpdate = user.list[ii].items;
                    break;
                }
            };
       };

       console.log('whichArrayToUpdate');

       User.update(
         {'list.listId':listURL},
         { $set: {
             'list.$.items': whichArrayToUpdate
           }
         },  (err) => {
         if (err) { return next(err); }
         //req.flash('success', { msg: 'Item added.' });
         ...
         console.log('req.body.itemName worked??');
       });
    });
  }
